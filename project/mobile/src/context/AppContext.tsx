...
export const AppProvider: React.FC<AppProviderProps> = ({ children }) => {
  ...
  useEffect(() => {
    const loadUser = async () => {
      try {
        const savedUser = await AsyncStorage.getItem('feedflow_user');
        if (savedUser) {
  ...
  const login = async (username: string) => {
    ...
    try {
      await AsyncStorage.setItem('feedflow_user', JSON.stringify(user));
  ...
  const logout = async () => {
    try {
      await AsyncStorage.removeItem('feedflow_user');
